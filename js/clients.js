...
function createBlocks(rows) {
	...
	clients.selectAll(".block-row").remove();

	let row_containers = clients.selectAll(".block-row")
		.data(rows)
		.enter()
		.append("div")
	...
	clients.transition().duration(500)
		.style("opacity", 1);
}

function createGlobals() {
	d3.csv("../assets/data/clients_lookup.csv", function(data, i) {
		metadata.push(data);
		metadata_dict[data.PlainLanguage] = data.Tag;
	})
}

function createClients(filters) {
	...
	.then(function(d) {
		d3.csv("../assets/data/client_tags.csv", function(data, i) {
			client_infos.push(data);
		})
		.then(function(d) {

			if (filters.length > 0) {
				client_infos = client_infos.filter(function(d) {
			...
			for (let i = 0; i < client_infos.length; i++) {

				if (i % blocks_per_row == 0) {
					rows.push([client_infos[i]]);
				}
				else {
					let j = i;
					...
					rows[j/blocks_per_row].push(client_infos[i]);
				}
			}
...
createGlobals();
createClients(curr_filters);

// Need to make the filter bolding persisted (refer back to curr_filters list)
